...
/*   Updated: 2022/07/13 15:24:24 by marlean          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int shadow_plane(t_data *data, t_dist *dist, t_coord *ray)
{
	...
	while (i < data->objects.nplane)
	{
		...
		if (dist_shadow > 0.0f && dist_shadow < 1.0f)///проверяемый луч прям до источника света, т.еб если что-то загораживает, то дистанцию надо умножить на число меньше 1
			return (1);
		i++;
	}
	return (0);
