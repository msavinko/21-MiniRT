...
/*   Updated: 2022/07/23 14:35:53 by mcherrie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_screen(t_data *data)
{
	...
	o = vector_addition(data->scene.camera.view_point,
			data->scene.camera.orient_vector);
	r.x = o.x;
	r.z = o.z;
	r.y = o.y + data->screen.height / 2;
	d.y = o.y;
	d.z = o.z + data->screen.width / 2 * (data->scene.camera.view_point.x - o.x);
	if (data->scene.camera.view_point.x - o.x == 0)
		d.x = o.x - data->screen.width / 2;
	else
		d.x = o.x - (d.z - o.z) * (data->scene.camera.view_point.z - o.z) / (data->scene.camera.view_point.x - o.x);
	data->screen.start = vector_addition(vector_subtract(r, o),
		vector_subtract(d, o));
	data->screen.start = vector_addition(data->screen.start, o);
	...
	vector_multiply(&data->screen.step_w, 2 / WIDTH);
	printf("\ndata->screen.step_w.x: %f data->screen.step_w.y: %f data->screen.step_w.z: %f\n", data->screen.step_w.x, data->screen.step_w.y, data->screen.step_w.z);
	data->screen.step_h = vector_subtract(d, data->screen.start);
	vector_multiply(&data->screen.step_h, 2 / HEIGHT);
...
// 	d.y = o.y;
// 	if (o.z < 0)
// 	d.z = o.z - data->screen.width / 2.0f/ sqrtf(pow((o.z - data->scene.camera.view_point.z) / (o.x - data->scene.camera.view_point.x), 2) + 1.0f);
// 	else if(o.z > 0)
// 	d.z = o.z + data->screen.width / 2.0f/ sqrtf(pow((o.z - data->scene.camera.view_point.z) / (o.x - data->scene.camera.view_point.x), 2) + 1.0f);
// 	d.x = o.x + (d.z - o.z) / (data->scene.camera.view_point.z - o.z);
// 	r.x = o.x;
// 	// такая же фигня с двумя r.z
